...
function respond() {
    ...
    botMention - /^\/mention/i; // responds with a message mentioning the user.
...
// cw
        ...
        this.res.end();
// mention
    } else if (request.text && botMention.test(request.text)) {
        var someText = request.name;
        this.res.writeHead(200);
        postMention(someText);
        this.res.end();
    } else {
